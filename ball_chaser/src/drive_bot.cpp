...
bool handle_drive_request(ball_chaser::DriveToTarget::Request& req, ball_chaser::DriveToTarget::Response& res)
{
  ...
  ROS_INFO("DriveToTarget Request received - linear_x:%1.2f, angular_z:%1.2f", (float)req.linear_x,
           (float)req.angular_z);
  ...
  res.msg_feedback = "Velocity set - linear_x: " + std::to_string(motor_command.linear.x) +
                     " and angular_z:  " + std::to_string(motor_command.angular.z);
...
int main(int argc, char** argv)
{
  ...
  ROS_INFO("Ready to send commands to my_robot");
